...
def mark_sent(theQueue, the_key_value: tuple, mark_type='set', expires=3600 * 24):
    ...
    '''
    if mark_type == 'set':
        return theQueue.RedisQ.set(the_key_value[0], the_key_value[1], expires)
    if mark_type == 'get':
...
def statistics(urlQ, to_addr_in):
    ...
    not_qty_sum, not_price, not_bsr, true_bsr, not_title, not_brand, not_img, not_rc, not_rrg = qty_retry(urlQ)
    ...
    field_msg_format = "已入库产品总数 %s, 其中:\n " \
                       "没有inventory %s, 占比 %.2f%%\n" \
                       "没有price %s, 占比 %.2f%%\n" \
                       "没有bsr %s, 占比 %.2f%%, 其中bsr解析失败 %s, 在没有bsr的asin中占比 %.2f%%\n" \
                       "没有title %s, 占比 %.2f%%\n" \
                       "没有brand %s, 占比 %.2f%%\n" \
                       "没有image %s, 占比 %.2f%%\n" \
    ...
    # 有品牌但是没有拿到的比例
    true_bsr_rate = true_bsr / not_bsr * 100
    # 没有标题的比例
    ...
    print(field_msg_format % (
            goods_num, not_qty_sum, not_qty_rate, not_price, not_price_rate,
            not_bsr, not_bsr_rate, true_bsr, true_bsr_rate,
    ...
    if goods_complete_rate >= 90:
        ...
        if not mark_sent(urlQ, (mark_key,), mark_type='get'):
            ...
            goods_sum, not_qty_sum, not_qty_rate, not_price, not_price_rate, not_bsr, not_bsr_rate, true_bsr, true_bsr_rate,
            not_title, not_title_rate, not_brand, not_brand_rate, not_img, not_img_rate,
            not_rc, not_rc_rate, not_rrg, not_rrg_rate
            ))
            send_func(msg_list, war_msg % (msg_type, goods_complete_rate), the_title_format, to_addr, from_addrs_name=from_addrs_name)
            # 标记发送状态, 以免重复发送
    ...
    if kw_complete_rate >= 90:
        ...
        if not mark_sent(urlQ, (mark_key,), mark_type='get'):
            ...
            send_func(msg_list, war_msg % (msg_type, kw_complete_rate), the_title_format, to_addr, from_addrs_name=from_addrs_name)
            # 标记发送状态, 以免重复发送
            mark_sent(urlQ, (mark_key, the_date.strftime('%Y%m%d %H:%M:%S')), mark_type='set')
        # 清空列表
        msg_list = []
    ...
    if tosell_complete_rate >= 90:
        ...
        if not mark_sent(urlQ, (mark_key,), mark_type='get'):
            ...
            tosell_retry_rate))
            send_func(msg_list, war_msg % (msg_type, tosell_complete_rate), the_title_format, to_addr, from_addrs_name=from_addrs_name)
            # 标记发送状态, 以免重复发送
            mark_sent(urlQ, (mark_key, the_date.strftime('%Y%m%d %H:%M:%S')), mark_type='set')
        # 清空列表
        msg_list = []
    ...
    if reviews_complete_rate >= 90:
        ...
        if not mark_sent(urlQ, (mark_key,), mark_type='get'):
            ...
            reviews_retry_rate))
            send_func(msg_list, war_msg % (msg_type, reviews_complete_rate), the_title_format, to_addr, from_addrs_name=from_addrs_name)
            # 标记发送状态, 以免重复发送
            mark_sent(urlQ, (mark_key, the_date.strftime('%Y%m%d %H:%M:%S')), mark_type='set')
        # 清空列表
        msg_list = []
...
def has_bsr(bsr_tuple_list, urlQ):
    bsr_true_list = []
    for bsr_tuple in bsr_tuple_list:
        asin = bsr_tuple[0]
        result = urlQ.RedisQ.get('%shasbsr' % (asin))
        if result:
            result = str(result, encoding='utf-8')
            if 'True' in result:
                bsr_true_list.append(asin)
    return bsr_true_list
...
def qty_retry(urlQ):
    ...
    if sql_times:
        ...
        true_bsr = len(has_bsr(bsr_tuple_list, urlQ))
        ...
        sql_brand = "select asin from public.amazon_product_data where asin_state!=0 and brand = '' and getinfo_tm >= %s;" % (
                    sql_times * 1000)
        brand_tuple_list = urlQ.retrieve_asin(sql_brand)
    ...
    return not_qty_sum, not_price, not_bsr, true_bsr, not_title, not_brand, not_img, not_rc, not_rrg

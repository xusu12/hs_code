...
def keyword_retry(kwQ):
    ...
    # sql_times = 0
    if sql_times:
        sql = "select kw, cid, aid, monitor_tm from public.amazon_keyword_monitor where state = 1 and crawler_tm < %s order by crawler_tm limit 8000;" % (
            sql_times)
        sql = "select kw, cid, aid, monitor_tm from public.amazon_keyword_monitor where state = 1 and crawler_tm < %s order by crawler_tm limit 80;" % (
            sql_times)
        print(sql)
        print('\nkw　重试进程　%s\n' % (sql_times))
...
def reviews_retry(urlQ):
    ...
    # sql_times = 0
    if sql_times:
        sql = "select asin, monitor_type, aid, comment_tm from public.amazon_product_monitor where state = 1 and comment_tm_crawler < %s and monitor_type >= 3 and monitor_type != 5 order by comment_tm_crawler desc limit 8000;" % (
            sql_times)
        print(sql)
        print('\nreviews　重试进程　%s\n' % (sql_times))
        url_tuple_list = urlQ.retrieve_asin(sql)
...
asin not in (select asin from public.amazon_product_data where getinfo_tm > %s and asin_state = 0) order by tosell_tm_crawler limit 8000;" % (
        sql_times, sql_times * 1000, sql_times * 1000)
        print(sql)
        print('\ntosell　重试进程　%s\n' % (sql_times))
        url_tuple_list = urlQ.retrieve_asin(sql)

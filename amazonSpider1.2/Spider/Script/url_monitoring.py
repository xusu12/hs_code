...
def get_monitor_tm(theQueue, tm_key=''):
    ...
    # 如果redis中没有 那么就从文件中读取时间戳数据
    if os.path.exists('%s/%s.tm' % (DATA_DIR, tm_key)):
...
def add_url_to_queue(theQueue, url_tuple_list, tm_key, url_type=''):
    ...
    if len(monitor_tm_list) >= 0:
        ...
        # 如果数据库中的最大时间戳比锚点时间戳要大  那么就重置redis和文件中存储的时间戳
        if tm_value > 0 and tm_value > sql_times:
            # 记录监控时间
            theQueue._set_key_value_to_string(tm_key, tm_value, key_type, overtime=86400)
            # 将时间戳锚点写入data目录下面
            with open('%s/%s.tm' % (DATA_DIR, tm_key), 'w', encoding='utf-8') as f:
...
def kw_monitor(kwQ):
    ...
    sql = "select kw, cid, aid, monitor_tm from public.amazon_keyword_monitor where state > 0 and monitor_tm > %s order by monitor_tm" % (
        sql_times)
    # sql = "select kw, cid, aid, monitor_tm from public.amazon_keyword_monitor where state > 0 order by monitor_tm";
    print(sql)
    print('\nkw　实时监控进程　%s\n' % (sql_times))
...
def goods_monitor(urlQ):
    ...
    sql = "select asin, monitor_type, aid, info_tm from public.amazon_product_monitor where state > 0 and monitor_type > 0 and info_tm > %s order by info_tm" % (
        sql_times)
    print(sql)
    print('\ngoods　实时监控进程　%s\n' % (sql_times))
...
def reviews_monitor(urlQ):
    ...
    tm_key = 'reviewsMaxTm'
    sql_times = get_monitor_tm(urlQ, tm_key)
    sql = "select asin, monitor_type, aid, comment_tm from public.amazon_product_monitor where state > 0 and comment_tm > %s and monitor_type >= 3 and monitor_type != 5 order by comment_tm" % (
        sql_times)
    print(sql)
    print('\nreviews　实时监控进程　%s\n' % (sql_times))
    url_tuple_list = urlQ.retrieve_asin(sql)
...
def tosell_monitor(urlQ):
    ...
    tm_key = 'tosellMaxTm'
    sql_times = get_monitor_tm(urlQ, tm_key)
    sql = "select asin, monitor_type, aid, tosell_tm from public.amazon_product_monitor where state > 0 and tosell_tm > %s and monitor_type >= 5 order by tosell_tm" % (
        sql_times)
    print(sql)
    print('\ntosell　实时监控进程　%s\n' % (sql_times))
    url_tuple_list = urlQ.retrieve_asin(sql)

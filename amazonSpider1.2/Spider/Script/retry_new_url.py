...
def get_init_updae_tm(theQueue):
    ...
    if tm_value:
        ...
        if re.match('^1[0-9]{9}$', result1):
            return int(result1)
    with open('%s/%s.tm' % (DATA_DIR, tm_key), 'r', encoding='utf-8') as f:
        tm = f.readline()
...
def keyword_retry(kwQ):
    ...
    if sql_times:
        sql = "select kw, cid, aid, monitor_tm from public.amazon_keyword_monitor where state = 1 \
and crawler_tm=0 and monitor_tm >= %s and kw not in (select kw from public.amazon_keyword_data where getinfo_tm > %s) \
order by monitor_tm limit 50;" % (sql_times, sql_times * 1000)
        print(sql)
        print('\nkw　重试进程　%s\n' % (sql_times))
...
and monitor_type > 0 and info_tm_crawler=0 and info_tm >= %s and asin not in (select asin from public.amazon_product_data where getinfo_tm > %s) \
order by monitor_tm limit 50;" % (sql_times, sql_times * 1000)
        print('\ngoods　重试进程　%s\n' % (sql_times))
...
def reviews_retry(urlQ):
    ...
    sql_times = get_init_updae_tm(urlQ)
    if sql_times:
        sql = "select asin, monitor_type, aid, comment_tm from public.amazon_product_monitor where state = 1 \
and comment_tm_crawler=0 and comment_tm >= %s and monitor_type >= 3 and monitor_type != 5 and asin not in \
(select asin from public.amazon_product_comment) order by comment_tm limit 30;" % (sql_times)
        print(sql)
        print('\nreviews　重试进程　%s\n' % (sql_times))
        url_tuple_list = urlQ.retrieve_asin(sql)
...
def tosell_retry(urlQ):
    ...
    sql_times = get_init_updae_tm(urlQ)
    if sql_times:
        sql = "select asin, monitor_type, aid, tosell_tm from public.amazon_product_monitor where state = 1 \
and tosell_tm_crawler=0 and tosell_tm >= %s and monitor_type >= 5 and asin not in\
(select asin from public.amazon_product_data_tosell where getinfo_tm > %s) order by tosell_tm limit 30;" % (sql_times, sql_times * 1000)
        print(sql)
        print('\ntosell　重试进程　%s\n' % (sql_times))
        url_tuple_list = urlQ.retrieve_asin(sql)

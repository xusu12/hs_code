...
def db_count_warning(to_addr='HSXinqun@aliyun.com'):
    ...
    msg_type = 'product'
    # 做标记所需的键
    mark_key = msg_type + the_date.strftime('%Y%m%d')
    if len(msg_list) > 0:
        # 遍历msg_list 分别取出每个过大的数据表
        for msg in msg_list:
            # 获取表名
            db_name = msg.split(' ')[0]
            # 做标记所需的键
            mark_key = msg_type + db_name + the_date.strftime('%Y%m%d')
            # 如果没有相应的标志位 说明两个小时内没有发送这个表对应的邮件
            if not mark_sent(urlQ, (mark_key,), mark_type='get'):
                #  针对每个表构造发送邮件的参数
                pt_time = pst_now()
                btm = date_now()
                msg_l = []
                msg_l.append(war_msg)
                msg_l.append(msg)
                msg_l.append(tm_format % (pt_time, btm))
                msg_string = '\n'.join(msg_l)
                resp = send_email_api(send_api, dict(ta=to_addr, ts=title_format % (pt_time), ms=msg_string))
                # 标记发送状态, 以免重复发送
                mark_sent(urlQ, (mark_key, the_date.strftime('%Y%m%d %H:%M:%S')), mark_type='set', expires=3600 * 2)
                print(resp.json())


if __name__ == '__main__':
    to_addr = 'HSLiulei@aliyun.com,hongz@intechannel.com,HSXinqun@aliyun.com,BruceX@selmuch.com'
    t_list = [

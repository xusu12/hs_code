...
class GoodsParser(BaseParser):
    ...
    def parser_goods(self, html_code, the_asin, monitor_type, ip='', ua='', info_log=None, debug_log=None,
        ...
        current_asin = self._get_current_asin()  # asin
        ...
        if dpre <= 0:
            dpre = price
        category = self._get_category()     # 分类
        print(category, type(category))
        fba=self._get_fba()
        offering_id = self._get_offering_id()
        if asin_state == 2:
            to_sell = 0

        goods_data = dict(
            asin=the_asin,  # asin
            asin_=current_asin, # 页面抓到的asin
            pasin=pasin,  # 父asin
            title=title,  # 标题
            image=image,  # 图片
            brand=brand,  # 品牌
        ...
        if not title and not image and price <= 0 and not brand:
            goods_datas = {}
        print('*' * 20)
        print('goods_datas_verify:')
        pprint(goods_datas)
        return goods_datas

    def get_head_html(self, html_code=None):
        if html_code is None:
            html_code = self.html_code
        table = self.soup.select('div[id="ppd"]')
        if len(table) < 1:
             table = self.soup.select('div[id="centerCol"]')
        if len(table) > 0:
        ...
        if len(result_list) > 0:
            html_code = str(tostring(result_list[0]), encoding='utf-8')
            return html_code
        return ''

    def get_buy_box_html(self, html_code=None):
        if html_code is None:
            html_code = self.html_code
        table = self.soup.select('div[id="buybox"]')
        if len(table) > 0:
            html_code = str(table[0])
            return html_code
        xpath_list = [
    ...
    def get_description_html(self, html_code=None):
        if html_code is None:
            html_code = self.html_code
        table = self.soup.select('div[id="detail-bullets"]')
        if len(table) < 1:
             table = self.soup.select('div[id="descriptionAndDetails"]')
        if len(table) > 0:
            html_code = str(table[0])
            return html_code
        xpath_list = [
    ...
    def _get_bsrRank(self, html_code=None):
        ...
        table = self.soup.select('table[id="productDetails_detailBullets_sections1"]')
        if len(table) < 1:
             table = self.soup.select('tr[id="SalesRank"]')
             if len(table) < 1:
                table = self.soup.select('li[id="SalesRank"]')
        if len(table) > 0:
            html_code = str(table[0])
            # print(html_code)
        xpath_list = [
    ...
    def _get_brand(self, html_code=None):
        ...
        # 使用上面的解析规则来提取 brand 可能会有一个规则拿到一个空的字符串  就不会再通过正确的解析规则拿到正确的数据
        if len(brand) > 0:
        ...
        # 如果拿到的brand信息是空字符串  那么就再使用xpath提取一次数据
        brand_xpath = [
            '//*[@id="bylineInfo"]/text()',
        ]
        brand = GoodsParser.get_new_data(xpath_list=brand_xpath, xpath_obj=self.xpath_obj)
    ...
    def _has_buy_button(self, html_code=None):
        ...
        if 'Add to Cart' in title or 'Add to Cart' in content:
            ...
            if len(result_list) > 0:
                ...
                pattern_list = [
                    re.compile('<input id="add-to-cart-button".*?>', re.S),
                ]
    ...
    def _get_review_count(self, html_code=None):
        ...
        # 商品页面还没有评论的提示信息  匹配到直接返回评论数0
        pattern_list = [
            re.compile('here are no customer reviews yet.'),
            re.compile('Be the first to review this item'),
        ...
        # 获取Additional Information标签  判断页面是否有评论数据
        divs = self.soup.select('div[class="a-section table-padding"]')
        if len(divs) > 0:
            html_code = str(divs[0])
            pattern_list = [
                re.compile('Be the first to review this item'),
                re.compile('0.0 out of 5 stars'),
            ]
            result_list = GoodsParser.get_new_data(pattern_list=pattern_list, html_code=html_code)
            # result_list大于0  说明页面没有评论
            if len(result_list) > 0:
                result1 = 0
                return int(result1)
        ...
        if len(review) < 1:
            table = self.soup.select('div[id="reviewSummary"]')
            # print(table)
            if len(table) > 0:
                html_code = str(table[0])
        ...
        if len(review) > 0:
            html = ''.join(review)
            pattern_list = [
                re.compile('[Se al]*([\d,]+) customer reviews'),
            ]
            result_list = GoodsParser.get_new_data(pattern_list=pattern_list, html_code=html)
            if len(result_list) > 0:
                review = result_list[0]
                review = ''.join(review.split(','))
                if review.isdigit():
                    result = int(review)
                    if result > 0:
                        return result
        divs = self.soup.select('div[id="reviewsMedley"]')
        if len(divs) > 0:
            html_code = str(divs[0])
            pattern_list = [
                re.compile('here are no customer reviews yet.'),
            ...
            if len(review) > 0:
                ...
                if review.isdigit():
                    ...
                    if result > 0:
                        return result
            pattern_list = [
                re.compile('See all ([\d,]+) customer reviews'),
                re.compile('[Se al]*([\d,]+) customer reviews'),
            ]
            result_list = GoodsParser.get_new_data(pattern_list=pattern_list, html_code=html_code)
            if len(result_list) > 0:
    ...
    def _get_review_rating(self, html_code=None):
        ...
        pattern_list = [
            ...
            re.compile('<span id="acrPopover".*?title="([\d\.]+) out of 5 stars">', re.S),
        ]
        result_list = GoodsParser.get_new_data(pattern_list=pattern_list, html_code=self.html_code)
        if len(result_list) > 0:
            result1 = result_list[0]
        ...
        '''↓↓↓ author: 徐骕 ↓↓↓'''
        # 添加 bs4 提取评分的span标签  使用正则来提取数据
        divs = self.soup.select('span[class="arp-rating-out-of-text a-color-base"]')
        pattern_list = [
            re.compile('<.*>([\d\.]+) out of 5 stars<.*>', re.S),
        ]
        if len(divs) > 0:
            html = str(divs[0])
            rating = GoodsParser.get_new_data(pattern_list=pattern_list, html_code=html)
            if len(rating) > 0:
                result1 = rating[0]
                print(10, float(result1) * 10)
                return float(result1) * 10

        # 使用bs4提取 Product information 信息框的页面代码  使用正则提取评分信息
        divs = self.soup.select('div[id="productDetails_feature_div"]')
        pattern_list = [
            re.compile('<br/*>\s*([\d\.]+)out of 5 stars\s*</td>', re.S),
        ]
        if len(divs) > 0:
            html = str(divs[0])
            rating = GoodsParser.get_new_data(pattern_list=pattern_list, html_code=html)
    ...
    def _get_review_5_percent(self, html_code=None):
        ...
        if len(percent) < 1:
            table = self.soup.select('div[class="a-meter 5star"]')
            if len(table) > 0:
                html_code = str(table[0])
    ...
    def _get_review_4_percent(self, html_code=None):
        ...
        if len(percent) < 1:
            table = self.soup.select('div[class="a-meter 4star"]')
            if len(table) > 0:
                html_code = str(table[0])
                percent = GoodsParser.get_new_data(xpath_list=percent_xpath, html_code=html_code)
    ...
    def _get_review_3_percent(self, html_code=None):
        ...
        if len(percent) < 1:
            table = self.soup.select('div[class="a-meter 3star"]')
            if len(table) > 0:
                html_code = str(table[0])
                percent = GoodsParser.get_new_data(xpath_list=percent_xpath, html_code=html_code)
    ...
    def _get_review_2_percent(self, html_code=None):
        ...
        if len(percent) < 1:
            table = self.soup.select('div[class="a-meter 2star"]')
            if len(table) > 0:
                html_code = str(table[0])
                percent = GoodsParser.get_new_data(xpath_list=percent_xpath, html_code=html_code)
    ...
    def _get_review_1_percent(self, html_code=None):
        ...
        if len(percent) < 1:
            table = self.soup.select('div[class="a-meter 1star"]')
            if len(table) > 0:
                html_code = str(table[0])
                percent = GoodsParser.get_new_data(xpath_list=percent_xpath, html_code=html_code)
    ...
    # 获取商品详情页分类信息的方法
    def _get_category(self, html_code=None):
        if not html_code:
            html_code = self.html_code
        # 获取详情页商品分类的xpath
        category = ''
        xpath_list = [
            '//ul[@class="a-unordered-list a-horizontal a-size-small"]//span[@class="a-list-item"]/a/text()', ]
        # 获取分类信息的数据
        result_list = GoodsParser.get_new_data(xpath_obj=self.xpath_obj, xpath_list=xpath_list)
        # 新列表保存提取后的分类信息
        if len(result_list) > 0:
    ...
    def _get_offering_id(self, html_code=None):
        if html_code is None:
            html_code = self.html_code
        xpath_list = [

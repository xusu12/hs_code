...
class ReviewsCrawler(BaseCrawler, Thread):
    ...
    def get_page_html(self, page_url, referer, **kwargs):
        page_html, cookies, page_is_error = self.get_html(page_url, referer)
    ...
    def get_reviews_html_list(self, asin, url, referer, **kwargs):
        print('\n' * 4, '*' * 20)
        print(self.get_reviews_html_list.__name__, asin, url, referer, **kwargs)
        print('*' * 20, '\n' * 4)
        self.html_list = []
        self.url_list = []
        self.is_error_list = []
        ...
        if self.looking_something(html) or not html:
            ...
            goods_html, cookiesObj, is_error = self.get_html(goodsUrl,  referer=goodsReferer, url_type=self.url_type, asin=asin)
        ...
        if reviewsUrl:
            ...
            html, cookiesObj, is_error = self.get_html(reviewsUrl, url_type=self.url_type, asin=asin)
    ...
    def download(self, asin_or_kw, url_dict):
        ...
        url = self.make_url(asin, url_type=url_type)[0]
        if url:
            startTime = return_PST().strftime("%Y-%m-%d %H:%M:%S")
            time1 = time_now()

            referer = self.make_url(asin, url_type='goods')[0]
            html_list, url_list, cookiesObj, is_error_lsit = self.get_reviews_html_list(asin, url, referer)
            i = -1
            is_not_found = None
            ...
            if is_not_found != 404:
                ...
                for html in html_list:
                    ...
                    if is_error:
                        ...
                        analyze = self.analyze_html(html, asin, url_dict, time1, startTime, html_type=url_type)
...
def reviews_start(urlQ, dataQ, kwQ, info_log, debug_log, i):
    ...
    while True:
        ...
        if urllen < 1:
            sys.exit()
        crawlers = [ReviewsCrawler(urlQ, dataQ, kwQ, info_log, debug_log) for i in range(t_num)]
...
if __name__ == '__main__':
    ...
    reviews_start(urlQ, dataQ, kwQ, info_log, debug_log, 1)

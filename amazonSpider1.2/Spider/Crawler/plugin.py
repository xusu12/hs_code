...
save_kw_list = [
    'womens aviator glasses',
    'slim for women',
    'ring video doorbell pro',
    'promise ring engraved sterling silver',
    'promise ring heart silver',
    'promise ring engraved',
    'sterling heart ring',
    'sterling silver couples rings',
    'sterling silver birthstone rings',
    'sterling silver heart rings',
    'silver birthstone rings',
    'promise jewelry for couples',
    'engraved birthstone promise ring',
    'engraved promise rings for her',
    'clothes',
    'sterling silver rings',
    'hand soap refill softsoap',
    'iphone charger and headphones adapter',
    'zip ties assorted industria',
    'nd filter 58mm gobe',
]
def save_kw_html(func):
    ...
    def wrap(*args, **kwargs):
        ...
        try:
            ...
            print('*' * 20)
            print(func.__name__, html_type, kw, len(html_list), type(html_list))
            print('*' * 20)
            ## 在 save_kw_list 中的 kw 才保存
            if kw in save_kw_list:
                '''注释掉上面一行后, 则是所有合法HTML都保存.'''
                if type(html_list) is list and html_list:
                    ...
                    for html in html_list:
                        ...
                        if not os.path.exists(base_dir):
                            os.mkdir(base_dir)
                        html_dir = os.path.join(base_dir, 'save_kw_html/')
                        if not os.path.exists(html_dir):
                            os.mkdir(html_dir)
                        save_dir = os.path.join(html_dir, datenow.strftime('%Y%m%d'))
                        if not os.path.exists(save_dir):
                        ...
                        file_path = os.path.join(save_dir,
                                                 '%s_%s_%s_%s.html' % (
                                                 kw, html_type, datenow.strftime('%Y%m%d_%H_%M_%S'), i))
                        # 将HTML写入文件
                        with open(file_path, 'w', encoding='utf-8') as f:
                            f.write(html)
                        i += 1

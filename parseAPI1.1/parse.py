...
# the_redis = Redis(**REDIS_CONFIG[BASE_TYPE])
R = the_redis()
# 缓存时间
...
class BaseParser:
    ...
    def get_new_data(xpath_obj=None, xpath_list=None, pattern_list=None, html_code=None, url_type='product'):
        if xpath_list and not xpath_obj:
            if html_code:
...
class GoodsParser(BaseParser):
    ...
    def _get_bsrRank(self, html_code=None):
        ...
        print('bsrRank_list1', bsrRank_list)
        if bsrRank_list:
    ...
    if data and type(data) is dict:
        ...
        if data_dict.get('bsr_info') or data_dict.get('feature'):
            ...
            try:
                ...
                if is_error:
                    request_msg_dict['request'][asin] = msg
                    PUBLIC_DATA['ret'] = 0
                    # print(type(html), html)
            except Exception as e:
                request_msg_dict['request'][asin] = '%s' % (e)
                PUBLIC_DATA['ret'] = 0
        ...
        if html:
            if not GoodsParser.is_RobotCheck(html):
                ...
                request_msg_dict['request'][asin] = 'robot check'
    else:
...
def parse():
    ...
    # request报错统一打包
    if request_msg_dict['request']:
        PUBLIC_DATA['msg'].append(deepcopy(request_msg_dict)) 
        request_msg_dict['request'] = {}

...
class BaseCrawler:
    ...
    # 解析源码
    def parser(self, html, html_type='', asin='', ip='', url='', ua='', info_log=None, debug_log=None, monitor_type=0,
        ...
        if html_type == 'goods':
            try:
                goods_datas, bsr_data = GoodsParser().parser_goods(html, asin, monitor_type, ip=ip, ua=ua,
                                                                   debug_log=debug_log, download_url=url,
                                                                   cookies=cookie)
        ...
        if html_type == 'tosell':
            try:
                tosell_info = TosellParser().tosell_parser(html, asin, tosellSum, ip=ip, download_url=url,
    ...
    def get_html(self, url, ua, ip, cookie, referer, url_type='', asin=''):
        html = ''
        cookiesObj = {}
        html, cookiesObj, is_error = get_html_useRequest(url, ua, ip, cookie, self.debug_log, referer, self.ipQ,
                                                         url_type=url_type, asin=asin)
        try:
    ...
    # html分析
    def analyze_html(self, html, cookie, cookiesObj, ip, asin_or_kw, url_dict, cookie_md5key, time1, startTime,
                     html_type=''):

...
from Crawler.plugin import save_kw_html
...
class KwParser(BaseParser):
    ...
    def kw_parser(self, html_code_list, keyword, cid, not_match=False):
        ...
        for html_code in html_code_list:
            ...
            j += 1
            # tm = int(time.time())
            # with open('./test_{}_{}.html'.format(j, tm), 'w', encoding='utf8') as f:
            #     f.write(html_code)
            ...
            for result in resultId:
                ...
                if asin:
                    ...
                    pprint(keyword_data_list)
            print(j, '数据长度:　', len(keyword_data_list))
            print('issp_sum:', issp_sum)
        i = 0
    ...
    def _get_asin(self, resultsObj, pr):
        ...
        asin_xpath = [
            ...
            '//*[@class="s-result-list sg-row"]/div/@data-asin',
        ]
        ...
        # 使用正则匹配
        # data-asin="B0753BTH7P"
        pattern_list = [
            re.compile('data-asin="(\w*)"', re.S),
        ]
        html_code = tostring(resultsObj)
        result_list = self.get_new_data(pattern_list=pattern_list, html_code=html_code)
        if len(result_list) > 0:
    ...
    def get_results_tag(html_code):
        ...
        xpath_list = [
            ...
            '//*[@class="s-result-list sg-row"]/div',
        ]
    ...
    def _get_title(self, resultsObj, pr):
        ...
        xpath_list = [
            ...
            '//*[starts-with(@class, "s-result-list sg-row")]//span[@class="a-size-base-plus a-color-base a-text-normal"]',
            '//span[@class="a-size-base-plus a-color-base a-text-normal"]/text()',
            '//a[starts-with(@class, "a-link-normal")]/span/text()',
        ]
    ...
    def _get_img(self, resultsObj, pr):
        # print('\nget img')  a-section aok-relative s-image-square-aspect
        result = ''
        xpath_list = [
            ...
            '//div[@class="a-section aok-relative s-image-square-aspect"]/img/@src',
            '//div[starts-with(@class, "a-section aok-relative ")]//img/@src',
        ]
    ...
    def _get_brand(self, resultsObj, pr):
        ...
        pattern_list = [
            ...
            re.compile(
                '<a.*?a-link-normal a-text-normal.*?><span.*?"a-color-secondary s-overflow-ellipsis s-size-mild">([\w -;]+)</span></a>',
                re.S),
        ...
        if len(result_list) > 0:
            ...
            if item:
                return item
        pattern_list = [
            re.compile('<a class="a-link-normal".*?><img.*?src="(https.*?brands.*?\.[jpg|png]+)".*?></a', re.S),
        ]
        result_list = self.get_new_data(html_code=html, pattern_list=pattern_list)
        if len(result_list) > 0:
            item = result_list[0].lstrip().strip()
        ...
        # 使用正则提取
        xpath_list = [
            '//div[@class="a-row a-size-base a-color-secondary"]/span/text()',
        ]
        result_list = self.get_data_from_xpthObj(resultsObj, xpath_list)
        if len(result_list) > 0:
            # result = result_list[0].split('by ')[1]
            res = re.findall(r'by ([\w -;]+)\'\,?', str(result_list))
            if len(res) > 0:
                item = result[0].lstrip().strip()
                if '&amp;' in item:
                    item = '&'.join(item.split('&amp;'))
                if len(item) > 128:
                    item = item.split(' ')[0]

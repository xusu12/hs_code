...
class GoodsParser(BaseParser):
    ...
    from Crawler.plugin import save_html
    from Crawler.verify_plugin import verify_plugin
    # @verify_plugin
    # 解析商品详情
    # @timer
    @save_html
    ...
    def _get_review_count(self, html_code=None):
        ...
        # 新增解析评论数的规则
        review_xpath = [
            '//*[@id="dp-summary-see-all-reviews"]/h2/text()',
        ]
        review = GoodsParser.get_new_data(xpath_list=review_xpath, xpath_obj=self.xpath_obj)
        if len(review) > 0:
            review = review[0].split(' ')[0]
            review = ''.join(review.split(','))
            if review.isdigit():
                result = int(review)
            if result > 0:
    ...
    def _get_review_rating(self, html_code=None):
        ...
        if self.soup:
            ...
            pattern_list = [
                re.compile('<.*>(.*) out of 5 stars<.*>', re.S),
            ]
            if len(divs) > 0:
                html = str(divs[0])
            ...
            pattern_list = [
                re.compile('<br/*>\s*(.*)out of 5 stars\s*</td>', re.S),
            ]
            if len(divs) > 0:
                html = str(divs[0])
                rating = GoodsParser.get_new_data(pattern_list=pattern_list, html_code=html)

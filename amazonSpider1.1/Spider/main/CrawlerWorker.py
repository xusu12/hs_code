...
# 进程数
from conf.setting import GOODS_P_NUM
from conf.setting import REVIEWS_P_NUM
from conf.setting import TOSELL_P_NUM
from conf.setting import KEYWORD_P_NUM
...
def goods(pNum, shell_str, num):
    msg = '%s(太平洋: %s)当前goodsCrawler.py工作进程 有%s个, 需要启动%s个\n %s' % (date_time, pst_date, num, pNum, shell_str)
    if pNum < 1:
        return print(msg)
    Crawler = os.path.join(BASE_DIR, 'Crawler/goodsCrawler.py')
...
def reviews(pNum, shell_str, num):
    msg = '%s(太平洋: %s)当前reviewsCrawler.py工作进程 有%s个, 需要启动%s个\n %s' % (date_time, pst_date, num, pNum, shell_str)
    if pNum < 1:
        return print(msg)
    Crawler = os.path.join(BASE_DIR, 'Crawler/reviewsCrawler.py')
    shell = 'nohup python3 %s > /dev/null 2>&1 &' % (Crawler)
    print(msg)
    for i in range(pNum):
        print(shell)
...
def keyword(pNum, shell_str, num):
    ...
    if pNum < 1:
        return print(msg)
    Crawler = os.path.join(BASE_DIR, 'Crawler/keywordCrawler.py')
    shell = 'nohup python3 %s > /dev/null 2>&1 &' % (Crawler)
    print(msg)
    for i in range(pNum):
        print(shell)
...
def tosell(pNum, shell_str, num):
    msg = '%s(太平洋: %s)当前tosellCrawler.py工作进程 有%s个, 需要启动%s个\n %s' % (date_time, pst_date, num, pNum, shell_str)
    if pNum < 1:
        return print(msg)
    Crawler = os.path.join(BASE_DIR, 'Crawler/tosellCrawler.py')
    shell = 'nohup python3 %s > /dev/null 2>&1 &' % (Crawler)
    print(msg)
    for i in range(pNum):
        print(shell)
...
def process_monitoring(num, shell_str, url_type=''):
    if url_type == 'goods':
        pNum = GOODS_P_NUM - num
        goods(pNum, shell_str, num)
    if url_type == 'reviews':
        pNum = REVIEWS_P_NUM - num
        reviews(pNum, shell_str, num)
    if url_type == 'tosell':
        pNum = TOSELL_P_NUM - num
        tosell(pNum, shell_str, num)
    if url_type == 'keyword':
        pNum = KEYWORD_P_NUM - num
